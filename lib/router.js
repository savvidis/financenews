...
PostsListController = RouteController.extend({
  ...
  postsOptions: function() { 
    return {sort:{PostDate:-1},fields: {Words:0,PostContent:0}, limit:this.postsLimit()}; 
  },
  nextPath: function() {
  ...
  //     var datenow = new Date();
      // var demo ={};
      // demo.prototype = {
      //   timeDiff: function () {return Math.abs(datenow.getTime() - Date.parse(this.PostDate));},
      //   hours : function () {
      //     console.log(this.timeDiff,this);
      //    return  Math.round(this.timeDiff / (1000 * 3600));},
      //   minutes : function () {return Math.round(this.timeDiff / (1000 * 60));},
      //   days : function () {return Math.round((this.timeDiff / (1000 * 3600 * 24)));},  
      //   score: function() {if (this.commentsfeed) { return Math.round(this.score  * (0.8 + this.CommentsNumber/ 30));}},
      //   gravity: function() {return Math.round(Math.pow(this.score,1+(this.quality/10))/Math.pow(Math.ceil(this.hoursdiff+1), gravitylevel[this.type]),2);}
      //   };
     ...
     //   _.map(this.data.posts, function(post) {
     //        post.__proto__ = Object.create(prop);
     //  });
  ...
  data: function() {
    var start = new Date(Date.now() - 1000*60*60*24*5); 
    var sortednewposts = Posts.find({ 'PostDate' : { $gte : start}},this.postsOptions()).fetch();
