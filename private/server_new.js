...
async.eachLimit(resp, concurrency, function (element, next) {
  index =+ 1;
   Fiber(function() { 
   list.push(ObjFromHeadline(element,index));
   Fiber.yield();
   }).run();
  next();
});
console.log("lis---t1111");

// _.each(resp, function (element, next) {
//   index =+ 1;
//    Fiber(function() { 
//     filePost(ObjFromHeadline(element,index));
//    Fiber.yield();
//    }).run();
// });
...
function ObjFromHeadline (element,index) {
      ...
      // Fiber(function() { 
        var hastitle = function () {
          ...
          var obj = {'type':newsource.type,'commentsfeed':newsource.commentsfeed, 'quality':newsource.quality,'score':initialscore,'source':newsource.source,'title': title,'url':correcturl,'submitDate':datenow};
           // Fiber(function() { 
           // Meteor.bindEnvironment(filePost (obj));
           //     }).run();
          return obj;
      // }).run();
}
    ...
    function filePost (obj){
      ...
      file.post = function () {
             console.log("234242424244");
            
             var getpost=  Posts.find({'url':obj.url}).fetch();
             return getpost; 

           };
      file.insert =function (obj,data) {
        if (data.length === 0) { data = null;}
          console.log("fsfdfdfsfsdfsdfdff");
        if (!!obj.commentsfeed && !!data) {
         console.log("++++ Post exist ? ",!!data);
         return;// makeUpdate(obj);
        } 
        else {
        if (!data){insertPost(extraxtData(obj));}
      //WE GET THE DATA FROM EACH POST
        } 
      };     

      // var datasync = Meteor.wrapAsync(file.post);

      // var data = datasync();
      // console.log("Sdfsfsfd");
      // var insertsync = Meteor.wrapAsync(file.insert);
      // var insert = insertsync(obj,data);
      // console.log(insert());
      // insert();
     
     // var post =Fiber(function() {  Posts.find({'url':obj.url}).fetch(); }).run();
